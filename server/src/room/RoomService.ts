import { Directions, Player, PlayerFactory, PlayerStates } from "../player/entities/Player";
...
export class RoomService {
    ...
    public addPlayer(player: Player): Room {
        ...
        const clientPlayer = PlayerFactory.buildPlayerClient(player.id, room.players.length + 1);

        room.players.push(clientPlayer);
        ServerService.getInstance().addPlayerToRoom(player.id, room.name, room.players.length);
